...
import { getHotels, getOwnedProperties, loginUser, registerUser, API_BASE_URL } from './communication';
...
// Read the current view and resource ID from the browser URL.
function readRoute() {
  const params = new URLSearchParams(window.location.search);
  return { page: params.get('page') || 'home', hotelId: params.get('hotelId') };
}
...
function App() {
  ...
  useEffect(() => {
    function restoreRoute() {
      ...
      setPage(route.page);
      if (route.hotelId) setHotel(hotels.find((item) => String(item.id) === route.hotelId) || null);
    }
  ...
  useEffect(() => {
    if (page !== 'trips' || user?.role !== 'hotel') return;
    getOwnedProperties().then(setOwnedProperties).catch((error) => showNotice(error.message));
  }, [page, user]);

  // Resolve URL resource IDs once hotels or owner properties are available.
  useEffect(() => {
    const route = readRoute();
    if (route.page === 'detail' && route.hotelId) {
      setHotel(hotels.find((item) => String(item.id) === route.hotelId) || null);
    }
    if (route.page === 'add-property' && route.hotelId) {
      setEditingProperty(ownedProperties.find((item) => String(item.id) === route.hotelId) || null);
    }
  }, [hotels, ownedProperties]);
  ...
  }), [filters, hotels, query.place, showAll]);

  // Navigate to a page while preserving its state in the URL.
  const navigate = (nextPage, values = {}) => {
    const params = new URLSearchParams({ page: nextPage, ...values });
    window.history.pushState({}, '', `?${params.toString()}`);
    setPage(nextPage);
  };
  const goHome = () => { navigate('home'); setHotel(null); setEditingProperty(null); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const openHotel = (item) => { setHotel(item); navigate('detail', { hotelId: item.id }); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const showNotice = (message) => { setNotice(message); window.setTimeout(() => setNotice(''), 3500); };
  const search = (event) => { event.preventDefault(); setShowAll(false); setFilters((old) => ({ ...old, place: query.place })); document.getElementById('stays')?.scrollIntoView({ behavior: 'smooth' }); };
  const submitAuth = async (event) => {
    event.preventDefault();
    const data = new FormData(event.currentTarget);
    const payload = { name: data.get('name') || 'Guest', email: data.get('email'), password: data.get('password'), role: data.get('role') || 'user' };
...
function Auth({ mode, onMode, onClose, onSubmit }) { return <div className="modal-bg" onMouseDown={(e) => e.target === e.currentTarget && onClose()}><div className="modal"><button className="close" onClick={onClose}>×</button><p className="eyebrow dark">WELCOME TO JAINSTAY</p><h2>{mode === 'login' ? 'Good to see you.' : 'Start your stay story.'}</h2><p className="modal-copy">{mode === 'login' ? 'Sign in to manage your reservations and save places for later.' : 'Create an account to book stays or list your property.'}</p><form onSubmit={onSubmit}>{mode === 'register' && <><Field label="Your name"><input name="name" required placeholder="Aarav Sharma" /></Field><Field label="I want to"><select name="role" defaultValue="user"><option value="user">Book stays</option><option value="hotel">List my property</option></select></Field></>}<Field label="Email address"><input name="email" type="email" required placeholder="you@example.com" /></Field><Field label="Password"><input name="password" type="password" minLength="8" required placeholder="8 characters minimum" /></Field><button className="primary" type="submit">{mode === 'login' ? 'Sign in' : 'Create account'} <span>→</span></button></form><button className="switch" onClick={() => onMode(mode === 'login' ? 'register' : 'login')}>{mode === 'login' ? 'New to jainStay? Create an account' : 'Already have an account? Sign in'}</button></div></div>; }
function Trips({ user, properties, onHome, onAddProperty, onEditProperty, onSignOut }) {
  if (user?.role === 'hotel') {
    return <main className="trips content"><div className="trips-head"><div><p className="eyebrow dark">PROPERTY PARTNER</p><h1>Your properties.</h1></div><div className="trips-actions"><button className="outline" onClick={onAddProperty}>Add property</button><button className="outline" onClick={onSignOut}>Sign out</button></div></div>{properties.length ? <div className="owner-property-grid">{properties.map((property) => <article className="owner-property-card" key={property.id}><img src={property.images?.[0]?.path || 'https://images.unsplash.com/photo-1601918774946-25832a4be0d6?auto=format&fit=crop&w=900&q=80'} alt={property.name} /><div><div className="owner-property-title"><h2>{property.name}</h2><span>{property.status}</span></div><p>{property.address}</p><small>{property.rooms?.length || 0} rooms · {property.images?.length || 0} images</small><button className="text-button property-edit-button" onClick={() => onEditProperty(property)}>Edit details ↗</button></div></article>)}</div> : <div className="empty"><div>✦</div><h2>Your first property<br /><i>starts here.</i></h2><p>You have not listed a property yet. Add one and start building your place on jainStay.</p><button className="primary" onClick={onAddProperty}>List a property <span>→</span></button></div>}</main>;
  }
